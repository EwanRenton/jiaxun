...
<div class="rightfix">
  ...
  <div class="menberlist">
      ...
      <?php if(is_array($vip)): foreach($vip as $key=>$val): ?><div class="menber_row">
  			...
  			<div class="menber_col7 f menber_col">
  				<p class="caozuo_con"><label class="look_up" vip="<?php echo ($val["vip_id"]); ?>">查看资料</label>/<label onclick="javascript:window.location.href='<?php echo U('/index.php/Admin/Member/delVip',array('vip_id'=>$val['vip_id']));?>'" class="delet_row">删除</label>/<?php if($val['vip_state'] == 1): ?><label class="stop_use" vid="<?php echo ($val["vip_id"]); ?>">停用账号</label><?php else: ?><label class="enable_use" vid="<?php echo ($val["vip_id"]); ?>">启用账号</label><?php endif; ?></p>
  			</div>
  			<div class="viplook">
				<div class="viptop">
...
<script type="text/javascript">
    ...
    if(adm_power.indexOf('会员列表') === -1){
      	...
      	$('.stop_use,.enable_use').click(function(){
			...
			$.post('/index.php/Admin/Member/state',{'id':id},function(data){
				if(data.res){
