...
<div class="rightfix">
  	<a class="home1"></a><a class="home_pass">图片管理</a><a class="home_now">banner管理</a><img src="/Public/Admin/images/addimg.jpg" alt="" onclick="window.location.href='/index.php/Admin/Photo/add'" class="addimg"/>
  	<div class="clear"></div>
  	<div class="menberlist">
		...
		<div class="menber_con">
			...
			<div class="menber_title">
  			...
  			<div class="clear"></div>
  			<?php if(is_array($banner)): foreach($banner as $key=>$val): ?><div class="photo_row">
  				...
  				<div class="photo_td5 f"><input type="text" value="<?php echo ($val["ban_gourl"]); ?>" banid="<?php echo ($val["ban_id"]); ?>"></div>
  				<div class="photo_td6 f"><a href="<?php echo U('/index.php/Admin/Photo/bandel',array('ban_id'=>$val['ban_id']));?>">删除</a></div>
  			</div>
...
if(adm_power.indexOf('banner管理') === -1){
	...
	$('.photo_td3 input').blur(function(){
		...
		$.post('/index.php/Admin/Photo/changeName',{'name':name,'id':id},function(data){
			if(data){
				return true;
			}
		});
	...
	$('.photo_td4 input').blur(function(){
		var order = $(this).val();
		var id = $(this).attr('banid');
		$.post('/index.php/Admin/Photo/changeOrder',{'order':order,'id':id},function(data){
			if(data){
				return true;
			}
		});
	...
	$('.photo_td5 input').blur(function(){
		var gourl = $(this).val();
		var id = $(this).attr('banid');
		$.post('/index.php/Admin/Photo/changeGourl',{'gourl':gourl,'id':id},function(data){
			if(data){
				return true;
			}
		});
...
$('#check_').change(function(){
	var bp_id = $(this).val();
	var url = '/index.php/Admin/Photo/index/bp/'+bp_id;
