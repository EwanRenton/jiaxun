...
class LinkController extends CommonController {
    public function img_link(){
        ...
        if(IS_POST){
            ...
            if($res){
               echo "<script>window.location.href='/Admin/Link/img_link';</script>";exit;
            }else{
    ...
    public function img_add(){
        ...
        if(IS_POST){
            ...
            if(!$info) {// 上传错误提示错误信息
                ...
                if($res){
                    echo "<script>window.location.href='/Admin/Link/img_link';</script>";
                }else{
	...
	public function words_link(){
        ...
        if(IS_POST){
            ...
            if($res){
               echo "<script>window.location.href='/Admin/Link/words_link';</script>";exit;
            }else{
                echo "<script>alert('删除失败');window.location.href='/Admin/Link/words_link';</script>";exit;
            }
        }
        $data = array(
                'bp_id'=>$bp_id,
    ...
    public function words_del(){
        ...
        if($res){
            echo "<script>window.location.href='/Admin/Link/words_link';</script>";
        }else{
            echo "<script>alert('删除失败');window.history.go(-1);</script>";
        }
    }
